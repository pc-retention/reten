...
import { clientPurchases, clients } from '../lib/testData';
...
export default function RemindersPage() {
  ...
  const allReminders = clientPurchases.map(p => ({
    ...p,
    client: clients.find(c => c.client_id === p.client_id),
  }));

  const todayReminders = allReminders.filter(r => {
    const rd = parseISO(r.reminder_date);
    return isToday(rd) && !r.reminder_sent;
  });

  const overdueReminders = allReminders.filter(r => {
    const rd = parseISO(r.reminder_date);
    return isBefore(rd, today) && !r.reminder_sent;
  });
